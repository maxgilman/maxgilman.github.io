...
class newParticle{
    constructor(x,y,size,color,type,momentum,friction){
        ...
        this.moveParticle = function(particle){
            ...
            particle.momentum=dividePoint(particle.momentum,particle.friction);
...
function newPowerUpPreset(PFType,isEffect){
    ...
    switch(PFType){
        ...
        break
        case 5:
            powerUp = new newMajorPowerUp(PFType,'Push Bullets and Enemies Away','#FFB475',function (thisEnemy,thisPowerUp){thisPowerUp.coolDown-=deltaTime},function(thisEnemy,thisPowerUp,gunAngle){
                thisPowerUp.coolDown=thisPowerUp.coolDownMax;
                screenShake+=3;
                let enemyRoomEnemies = thisEnemy.enemyRoom.enemies;
                let bullet=new newBullet(0,0,40,0,'#FFB475',20,40,1,10,1,new newPoint(0,0),thisEnemy,0,'',20,(Math.PI*2)/2);
                shootBullet(bullet,0,thisEnemy,bullet.bulletSpreadNum,bullet.shotSpread,0,thisEnemy,thisEnemy.size);
                for (enemy of enemyRoomEnemies){
                    if (enemy===thisEnemy||enemy.team===2){
                        continue;
                    }
        ...
        break
        case 11:
            powerUp = new newMinorPowerUp(PFType,'Explosives Deal 1.5X Damage','#FF6E6E',function (majorPowerUp,thisPowerUp){
        ...
        break
        case 12:
            powerUp = new newMinorPowerUp(PFType,'2X for Weapon Bound to Right Click','#FFAC01',function (majorPowerUp,thisPowerUp){
        ...
        break
        case 17:
            powerUp = new newMinorPowerUp(PFType,'After Gaining Health, Deal 3X Damage for 10 Seconds','#f0c013',function (majorPowerUp,thisPowerUp){
                if(enemies[0].health>thisPowerUp.var1){
                    thisPowerUp.var2+=300;
                }
                thisPowerUp.var1=enemies[0].health;
        ...
        case 37:
            powerUp = new newMajorPowerUp(PFType,'Lightning Staff','yellow',function (thisEnemy,thisPowerUp){thisPowerUp.coolDown-=deltaTime},function(thisEnemy,thisPowerUp,gunAngle){
                ...
                bullet.effect = function(enemyHit,thisBullet){
                    screenShake+=1;
                    if (undefined===enemyHit.statusEffects.find((effect)=>effect.type===1)){
                        enemyHit.statusEffects.push({
        ...
        break
        case 39:
            powerUp = new newMinorPowerUp(PFType,'1/2 Cooldown on everything, But Everything Does 2/3 Damage','#07ed5f',function (majorPowerUp,thisPowerUp){
                majorPowerUp.coolDownMax/=2;
        ...
        break
        case 44:
            powerUp = new newMajorPowerUp(PFType,'Soul Summoner','#004700',function (thisEnemy,thisPowerUp){thisPowerUp.coolDown-=deltaTime},function(thisEnemy,thisPowerUp,gunAngle){
                thisPowerUp.coolDown=thisPowerUp.coolDownMax;
                screenShake+=3;
                let soulNum = 0;
                ...
                let i=soulNum;
                while(i<16&&i<(soulNum+6)){
                    i++;
                    let enemy = newEnemyPreset(addToPoint(dupPoint(thisEnemy),Math.random()-.5,Math.random()-.5),9,1,'',thisPowerUp.damage,thisEnemy)
                    enemies.push(enemy);
        ...
        case 45:
            powerUp = new newMajorPowerUp(PFType,'Pistol','#DEDEDE',function (thisEnemy,thisPowerUp){thisPowerUp.coolDown-=deltaTime},function(thisEnemy,thisPowerUp,gunAngle){
                thisPowerUp.coolDown=thisPowerUp.coolDownMax;
                screenShake+=3;
                let bullet = new newBullet(0,0,30,0,'red',60,5,0,thisPowerUp.range,thisPowerUp.bulletKillPower,new newPoint(0,0),thisEnemy,thisPowerUp.damage,'',1,1);
                shootBullet(bullet,gunAngle,thisEnemy,bullet.bulletSpreadNum,bullet.shotSpread,0,thisEnemy,thisEnemy.size);
            },25,1,30,1);
        break
        case 46:
...
function newEnemyPreset(pos,PFType,power,message,enemyPower,target){
    ...
    switch (PFType){
        ...
        case 11:
            //Dashing boss
            enemy = new newEnemy(pos.x,pos.y,40,40,'#4C5C7D',11,target,30/*maybe scale this with difficulty*/,1+(10*Math.pow(2,enemyPower/1.5)),'',function(touchedEnemy,thisEnemy,enemiesToRemove){
                if ((touchedEnemy.invinceable<1)&&(touchedEnemy.team!=thisEnemy.team)&&touchedEnemy.team!=2){
                    touchedEnemy.health--;
                    touchedEnemy.invinceable=touchedEnemy.maximumInvinceable+10;
                }
        ...
        case 12:
            //teleporting boss
            enemy = new newEnemy(pos.x,pos.y,0,40,'#37A7AA',12,target,60/*maybe scale this with difficulty*/,1+(10*Math.pow(2,enemyPower/1.5)),'',function(touchedEnemy,thisEnemy,enemiesToRemove){});
        break
        case 14:
        ...
        case 36: //this is the actual bomb
            ...
            enemy.gunCooldown=enemy.gunCoolDownMax;
        break
        //the boss and rider health is hard to get right and needs to be rebalenced
        case 37:
        ...
        case 38:
            ...
            enemy = new newEnemy(pos.x,pos.y,3+(enemyPower/2),20,'green',PFType,target,45-(enemyPower*3),(5*Math.pow(2,enemyPower/1.5)),'',undefined,undefined,undefined,undefined,undefined,undefined,undefined,undefined,20+enemyPower);
        break
    }
...
class newBullet {
    constructor(x,y,speed,direction,color,tailLength,visualWidth,type,maximumTime,enemyKillPower,lastPosition,owner,damage,label,bulletSpreadNum,shotSpread){
        ...
        this.boringEffect = function(enemyHit,thisBullet){
            thisBullet.enemiesHit.push(enemyHit);
            thisBullet.enemiesLeft--;
            if (thisBullet.owner===enemies[0]){
                if (Math.random()<onHitHPDropChance){
                    dropEnemyLoot(enemyHit,enemy.enemyRoom,1/2);
                }
            }
...
gunImages.granadeLauncher.src='Gun_Images/Granade_Laucher.png';
    ...
    for (enemyRoom of enemyRooms){
        ...
        for (enemy of enemyRoom.enemies){
            ...
            if (enemy.PFType===1||enemy===enemies[0]){
                ...
                if (guns.includes(powerUpsGrabbed[0].PFType)){
                    firstSlotGun=true;
                    gunNum++;
                }
                if (guns.includes(powerUpsGrabbed[1].PFType)){
                    secondSlotGun=true;
                    gunNum++;
                }
                let image;
                if (secondSlotGun||firstSlotGun){
                    if (secondSlotGun&&firstSlotGun){
                        for (let i=0;i<2;i++){
                            image=gunImages[guns.findIndex((gunCheck)=>gunCheck===powerUpsGrabbed[i].PFType)];//guns and gunImages are lined up together
                            ctx.save();
                        ...
                        if (firstSlotGun){
                            image=gunImages[guns.findIndex((gunCheck)=>gunCheck===powerUpsGrabbed[0].PFType)];
                        }else{
                            image=gunImages[guns.findIndex((gunCheck)=>gunCheck===powerUpsGrabbed[1].PFType)];
                        }
                        ctx.save();
                        ctx.translate(gunPos.x,gunPos.y);
                        ctx.rotate(-gunAngle);
                ...
                if (enemy.gunCooldown>0){
                    if (((enemy.gunCooldown<10)&&((enemy.gunCooldown%6)<2))){
                        ctx.beginPath();
                        ctx.arc(screenEnemyPos.x,screenEnemyPos.y,100,0,Math.PI*2);
                        ctx.fillStyle='red';
                        ctx.fill();
                        ctx.stroke();
                    }
...
function generateRooms(targetNumOfRooms,finalDifficulty,eligibleMajorPowerUps){
    ...
    for (room of finishedRooms){
        i++;
        walls = walls.concat(generateRoom(room.up,room.right,room.down,room.left,room.roomPos,i-2,(((i-3)/(targetNumOfRooms))*(finalDifficulty-1))+1,eligibleMajorPowerUps));
    }
...
function enemyMovement(enemiesToRemove,skipPlayer){
    ...
    for (mainEnemyRoom of enemyRooms){
        ...
        for (let i =0;i<enemyRoom.length;i++){
            ...
            if(enemy.health<=0||(enemy.PFType===9&&firstEnemy===undefined)){
            ...
            if (enemy.PFType===0||enemy.PFType===4||enemy.PFType===15||(enemy.PFType===37&&enemy.target.team===0)){
                ...
                if (enemy.target.deleted===true){
                    enemy.PFType=4;//this is the other green enemy. Effectively, the enemy "dismounts"
                    enemy.target=enemies[0];
                }else{
                    enemy.x=enemy.target.x
                    enemy.y=enemy.target.y
...
function gunEnemyMovement(target){
    ...
    for (mainEnemyRoom of enemyRooms){
        ...
        for (let i =0;i<enemyRoom.length;i++){
            ...
            }else if(enemy.PFType===10||enemy.PFType===4||enemy.PFType===5||enemy.PFType===3||enemy.PFType===2||enemy.PFType===17||enemy.PFType===15||enemy.PFType===37||enemy.PFType===38){
...
function drawPermanentSlots(permanentLeftX,permanentVeryTopY){
    let permanentBottomY = 5;
    let permanentTopY = permanentVeryTopY-30;
    let permanentMidX = permanentLeftX+15;
    ctx.beginPath();
    ctx.arc(permanentMidX,(c.height-(20))-permanentBottomY,20,0,Math.PI*2);
    ...
    ctx.fill();
    ctx.beginPath();
    for (let i=0;i<permanentMinorPowerUps.length+minorPowerUpSpace;i++){
        ctx.arc(permanentMidX,c.height-(10)-((i+.5)*30),13,0,Math.PI*2);
    }
    ctx.fillStyle='white';
    ctx.fill();

    ctx.drawImage(powerUpIconImage,permanentLeftX+3,(c.height-(permanentVeryTopY-25)+permanentBottomY),25,25);
    ...
    let metrics = ctx.measureText(message);
    ctx.fillText(message,permanentMidX-(metrics.width/2),c.height-permanentTopY-10);
}
function drawHUD(){
    ctx.fillStyle='black';
    ...
    let permanentVeryTopY = permanentLeftX+((minorPowerUpSpace+permanentMinorPowerUps.length+2)*30);
    if (showHUD.minorPowerUps){
        drawPermanentSlots(permanentLeftX,permanentVeryTopY);
    }
        ...
        switch (k){
            ...
            case 5:
                if (heldPowerUp===undefined){
                    ...
                    if (enemies[0].enemyRoom.roomNum===0){
                        let leftTextList = [];
                        addText('Left Click Slot',new newPoint(5,25),'24px serif','black',1000000,leftTextList);
                        addRect(new newPoint(5,c.height-120),150,30,'white',leftTextList,false);
            
                        let rightTextList = [];
                        addText('Right Click Slot',new newPoint(5,25),'24px serif','black',1000000,rightTextList);
                        addRect(new newPoint(130,c.height-85),165,30,'white',rightTextList,false);
                        ...
                        let textList = [];
                        addText('Drag Modifiers into These Smaller Slots',new newPoint(5,25),'24px serif','black',1000000,textList);
                        ...
                        if ((frameNum%40)<20){ //this flashing won't work at different frame rates
                            ctx.beginPath();
                            for (let i=0;i<minorPowerUpSpace;i++){
                                ctx.arc(permanentLeftX+15,c.height-25-(30*i),11,0,Math.PI*2);
                            }
                            ctx.fillStyle='blue';
                            ctx.fill();
                        }
        ...
        for(let i=0;i<powerUpList.length;i++){
            ...
            switch (k){
                ...
                case 2:
                    ...
                    break
                case 3:
                    i=drawPowerUp(examplePowerUp,i,-.33,onClick,powerUpList,permanentLeftX+15,false,false,false,true);
                    break
                case 4:
                    i=drawPowerUp(examplePowerUp,i,minorPowerUpSpace-.33,function(i,powerUpList){return i},powerUpList,permanentLeftX+15,false,false,false,true,true);
                    break
                case 5:
...
function drawPowerUp(powerUp,numOnScreen,verticalNumOnScreen,onClick,powerUpList,leftX,isBullet,powerUpSelect,skipDrawingPowerUp,stackVertical,addLockOverlay){
    ctx.beginPath();
    ...
    if (!skipDrawingPowerUp){
        ...
        ctx.globalAlpha=1;
        if(addLockOverlay){
            ctx.drawImage(lockImage,iconPos.x-(iconSize*1),iconPos.y-(iconSize*1),iconSize*2*1,iconSize*2*1);
        }
    }
    iconPos.x-=10;
    if (!powerUpSelect){
    ...
    if (findDis(iconPos,mouse)<iconSize){
        ...
        for(let i=0;i<4;i++){
            ...
            switch(i){
                ...
                case 3:
                    ...
                    text = 'Cooldown: '+roundTo2(examplePowerUp.coolDownMax/30,100)+' '+secondsText;
                    break
            }
            let metrics = ctx.measureText(text);
        ...
        let powerUpPos = new newPoint(Math.max(5,Math.min(mouse.x-(maxTextWidth/2),c.width-(maxTextWidth+5))),Math.min(mouse.y-20,c.height-31))
        if (mouseClickUsed){
            mouseClickUsed=false;
            ...
            if (textToDraw.length>0){
                ...
                addRect(addToPoint(powerUpPos,-5,-rectHeight+10),maxTextWidth+10,rectHeight,'white',textToDraw,true);
            }
        }
    }
    return numOnScreen
...
function drawShop(){
    ...
    /*const gradient = ctx.createLinearGradient(screenShopPos.x, screenShopPos.y, screenShopPos.x, screenShopPos.y+20);
    ...
    ctx.fillRect(screenShopPos.x,screenShopPos.y,10,50);
    ctx.fillRect(screenShopPos.x+90,screenShopPos.y,10,50);
    ctx.fillStyle = gradient;
    //ctx.fillRect(screenShopPos.x,screenShopPos.y,100,10);
    ctx.beginPath();
    ctx.moveTo(screenShopPos.x,screenShopPos.y);
    ctx.lineTo(screenShopPos.x+100,screenShopPos.y);
    ctx.lineTo(screenShopPos.x+90,screenShopPos.y+15);
    ctx.lineTo(screenShopPos.x+10,screenShopPos.y+15);
    //ctx.lineTo(screenShopPos.x,screenShopPos.y);
    ctx.fill();*/

    ctx.fillStyle='red';
    ctx.fillRect(screenShopPos.x,screenShopPos.y+60,100,50);
    ctx.fillStyle='black';
    ctx.font = '40px serif';
    ctx.fillText('Shop',screenShopPos.x+10,screenShopPos.y+95);
    ctx.fillStyle='blue';
    ctx.beginPath();
    ctx.arc(screenShopPos.x+50,screenShopPos.y+60,20,0,Math.PI,true);
    ctx.fill();
    ctx.stroke();
    ...
    ctx.beginPath();
    ctx.arc(screenShopPos.x+42,screenShopPos.y+50,4,0,Math.PI*2,true);
    ctx.arc(screenShopPos.x+58,screenShopPos.y+50,4,Math.PI,-Math.PI,true);
    ctx.fill();
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(screenShopPos.x+50,screenShopPos.y+55,4,0,Math.PI);
    ctx.stroke();
...
function updateShopPos(){
    ...
    if (firstEnemy===undefined&&enemies[0].enemyRoom.roomNum>1){
        let roomPos = turnRoomIntoRealPos(enemies[0].room)
        let enemyRoomWalls=enemies[0].enemyRoom.walls;
        shop.x=roomPos.x+75;
        shop.y=roomPos.y+100;
        let prospectWall = shiftWallsBy([new newWall(75,210,175,210)],roomPos.x,roomPos.y)[0];
        if (!(isSamePoint(enemyRoomWalls[0].first,prospectWall.first)&&isSamePoint(enemyRoomWalls[0].second,prospectWall.second))){
            enemyRoomWalls.splice(0,0,prospectWall);
        }
...
function repeat2(){
    ...
    if (mode===0||mode===4){
        ...
        ctx.drawImage(gunImages[2],(c.width/8),(2.3*c.height/4),gunImages[2].width*3,gunImages[2].height*3)
        ctx.fillText('Press 2 to use The Sniper',(c.width/3),(c.height/2));
        ctx.drawImage(gunImages[3],(1.2*c.width/3),(2.3*c.height/4),gunImages[3].width*3,gunImages[3].height*3)
